...
_kiwi.model.Channel = _kiwi.model.Panel.extend({
    initialize: function (attributes) {
        ...
        members.bind("add", function (member) {
            ...
            this.addMsg(' ', '== ' + _kiwi.global.i18n.translate('%s has joined').fetch(member.displayNick(true)), 'action join');
        }, this);
        ...
        members.bind("remove", function (member, members, options) {
            ...
            if (options.type === 'quit' && show_message) {
                this.addMsg(' ', '== ' + _kiwi.global.i18n.translate('%s has quit %s').fetch(member.displayNick(true), msg), 'action quit');
                ...
                if (!options.current_user_kicked) {
                    ...
                    if (show_message || options.current_user_initiated) {
                        this.addMsg(' ', '== ' + _kiwi.global.i18n.translate('%s was kicked by %s %s').fetch(member.displayNick(true), options.by, msg), 'action kick');
                    }
                } else {
                    this.addMsg(' ', '== ' + _kiwi.global.i18n.translate('You have been kicked by %s %s').fetch(options.by, msg), 'action kick');
                }
            } else if (show_message) {

                this.addMsg(' ', '== ' + _kiwi.global.i18n.translate('%s has left %s').fetch(member.displayNick(true), msg), 'action part');
            }
        }, this);
    }
});
