...
_kiwi.model.Channel = _kiwi.model.Panel.extend({
    initialize: function (attributes) {
        ...
        members.bind("add", function (member, members, options) {
            ...
            this.addMsg(' ', _kiwi.global.text_theme.styleText('client_models_channel_join', [member.displayNick(true)]), 'action join', {time: options.kiwi.time});
        }, this);
        ...
        members.bind("remove", function (member, members, options) {
            ...
            if (options.kiwi.type === 'quit' && show_message) {
                this.addMsg(' ', _kiwi.global.text_theme.styleText('client_models_channel_quit', [member.displayNick(true)], msg), 'action quit', {time: options.kiwi.time});
                ...
                if (!options.kiwi.current_user_kicked) {
                    ...
                    if (show_message || options.kiwi.current_user_initiated) {
                        this.addMsg(' ', _kiwi.global.text_theme.styleText('client_models_channel_kicked', [member.displayNick(true), options.kiwi.by, msg]), 'action kick', {time: options.kiwi.time});
                    }
                } else {
                    this.addMsg(' ', _kiwi.global.text_theme.styleText('client_models_channel_selfkick', [options.kiwi.by], msg), 'action kick', {time: options.kiwi.time});
                }
            } else if (show_message) {
                this.addMsg(' ', _kiwi.global.text_theme.styleText('client_models_channel_part', [member.displayNick(true)], msg), 'action part', {time: options.kiwi.time});
